...
## compute correlation
POS2 <- snp_asGeneticPos(CHR, POS, dir = paste0('/dcs04/nilanjan/data/ydun/ldpred2/tmp_dir/EUR/'), ncores = 2)
## indices in info_snp
ind.chr <- which(info_snp$chr == chr)
df_beta <- info_snp[ind.chr, c("beta", "beta_se", "n_eff")]
## indices in G
ind.chr2 <- info_snp$`_NUM_ID_`[ind.chr]
corr0 <- snp_cor(G, ind.col = ind.chr2[1:89000], ncores = 48, #size = ldradius)
                 infos.pos = POS2[ind.chr2[1:89000]], size = ldr) # default
